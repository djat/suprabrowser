...
public class DocumentRenderer implements Printable {
	...
	private static org.apache.log4j.Logger logger = ss.global.SSLogger
			.getLogger(DocumentRenderer.class);
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) {
		...
		// II
		this.jeditorPane.setSize((int) pageFormat.getImageableWidth(),
				Integer.MAX_VALUE);
		this.jeditorPane.validate();
		// III
		rootView = this.jeditorPane.getUI().getRootView(this.jeditorPane);
		// IV
		if ((this.scaleWidthToFit)
				&& (this.jeditorPane.getMinimumSize().getWidth() > pageFormat
						.getImageableWidth())) {
			scale = pageFormat.getImageableWidth()
					/ this.jeditorPane.getMinimumSize().getWidth();
		...
		graphics2D.translate(graphics2D.getClipBounds().getX(), graphics2D
				.getClipBounds().getY());
		// VIII
		Rectangle allocation = new Rectangle(0, (int) -this.pageStartY,
				(int) (this.jeditorPane.getMinimumSize().getWidth()),
				(int) (this.jeditorPane.getPreferredSize().getHeight()));
		// X
	...
	protected void printDialog() {
		if (this.pJob.printDialog()) {
			...
			try {
				...
				this.pageEndY = 0;
				this.currentPage = -1;
				logger.error("Error Printing Document", ex);
			}
		}
	}
				...
				if ((allocation.getBounds().getHeight() > clipRectangle
					...
					if (allocation.getBounds().getY() >= clipRectangle.getY()) {
						if (allocation.getBounds().getMaxY() <= clipRectangle
							...
							// IV
							if (allocation.getBounds().getY() < this.pageEndY) {
