...
public class CharTerminatedInputStream extends InputStream {
	...
	/**
	 * An array containing the last N characters read from the stream, where N
	 * is the length of the terminating character array
	 */
	private int buffer[];

	/**
	 * The number of bytes that have been read that have not been placed in the
	 * internal buffer.
	 */
	private int pos = 0;
	...
	public CharTerminatedInputStream(InputStream in, char[] terminator) {
		if (terminator == null) {
			throw new IllegalArgumentException(
					"The terminating character array cannot be null.");
		}
		if (terminator.length == 0) {
			throw new IllegalArgumentException(
					"The terminating character array cannot be of zero length.");
		}
		this.match = new int[terminator.length];
		this.buffer = new int[terminator.length];
		for (int i = 0; i < terminator.length; i++) {
			this.match[i] = (int) terminator[i];
			this.buffer[i] = (int) terminator[i];
		}
		this.in = in;
	}
	 ...
	 *             if the underlying stream returns -1 before the terminator is
	 *             seen.
	 */
	...
	/**
	 * Private helper method to update the internal buffer of last read
	 * characters
	 * 
