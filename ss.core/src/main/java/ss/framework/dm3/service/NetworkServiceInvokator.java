...
public class NetworkServiceInvokator implements IServiceInvokator {
	...
	/*public void ainvoke(String methodName, Class<?>[] parameterTypes,
			Serializable[] args, ICallbackHandler resultHandler)
			throws ServiceException {
		ServiceCommand command = new ServiceCommand( this.serviceClass, methodName, parameterTypes, args ); 
		try {
			resultHandler.onSuccess( command.execute( this.protocol, Serializable.class ) );
		} catch (CommandExecuteException ex) {
			resultHandler.onFail(ex);
		}
		throw new ServiceException( "HERE2 Service invokation failed. Command " + command );
	}*/
