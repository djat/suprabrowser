 ...
 * 
 */
class LightSWTBrowser extends Browser implements IBrowser {

	private static final Logger logger = SSLogger.getLogger(LightSWTBrowser.class);
	...
	public LightSWTBrowser(SupraBrowserProxy parent, int style, boolean belongToBrowserPane) {
		super(parent, style);
		...
		addProgressListener(new ProgressAdapter() {
			...
			public void changed(ProgressEvent pe) {
				...
				LightSWTBrowser.this.canComment = false;
			}
		});		
	}
	/**
	 ...
	 * @param arg1
	 */
	public LightSWTBrowser(SupraBrowserProxy parent, int style) {
...
//		}
	...
	private void addMainElementsToMenu( final Menu menu ){
		...
		if (!LightSWTBrowser.this.belongToSBrowserDocking) {
			...
			comment.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent arg0) {
					...
					if (selection != null
							...
							&& LightSWTBrowser.this.canComment) {
						...
						UserMessageDialogCreator
								.error(LightSWTBrowser.this.bundle
					...
					public void widgetSelected(SelectionEvent se) {
						new Thread() {
							public void run() {
								...
								SavePageWindow
										.showDialog(
												(SupraBrowser) LightSWTBrowser.this.parent,
												sphereOwner);
							}
						}.start();
						LightSWTBrowser.this.setFocus();
					}
				});
	...
	protected void onProgressCompleted(ProgressEvent event) {
		...
		LightSWTBrowser.this.parent.getMozillaBrowserController().injectJS();
		addAnchorsClickListener();
		invokeCompletedListeners();
		if(getMP() != null && LightSWTBrowser.this.showAllCommentedPlaces) {
		...
		LightSWTBrowser.this.canComment = true;
	}
